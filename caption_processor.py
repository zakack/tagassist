...
class CaptionProcessor:
  ...
  def caption_me(self, initial_prompt, image):
    ...
    try:
      ...
      hair_length = self.ask("Is her hair short or long?", image)
      p_hair = f"{hair_length} {hair_color}"
      # print(p_hair)
      ...
      p_style = self.ask("Between the choices professional, amateur, or POV; what is the style of the photo?", image)
      # print(p_style)

      p_clothing = self.ask("What is she wearing?", image)
      # print(p_clothing)

      #p_action = self.ask("What is she doing? Could be something like standing, stretching, walking, squatting, etc", image)
      #p_action = self.ask("What is she doing? Could be something like sleeping, standing, sitting, squatting, etc", image)
      p_action = self.ask("What is she doing? Could be something like standing, sitting, squatting, etc", image)
      # print(p_action)

      p_age = self.ask("Is she a young child or an adolescent?", image)
      # print(p_age)

      p_framing = self.ask("Would you consider this photo a close up, upper body shot, full body shot, or wide shot?", image)
      # print(p_framing)
      ...
      prompt = self.sanitise_caption(f"{p_action}, {p_style} {initial_prompt}, {p_age}, {p_hair} hair, {p_clothing}, {p_framing}, {p_setting}, {p_angle}")
